...
import { Clock, Eye, LoaderCircle, RotateCcw, Search, Send, WifiOff, X } from "lucide-react";
...
export default function MvpApp() {
  ...
  const addDraft = (point: NormalizedPoint) => {
    if (draft.length >= GAME_CONFIG.differenceCount || me?.submitted) return;
    const kinds: Difference["kind"][] = ["ADD", "COVER", "COLOR"];
    setDraft((current) => [...current, { id: `${game.match!.playerId}-${current.length + 1}`, kind: kinds[current.length]!, region: { ...point, radius: 0.05 } }]);
  };

  return (
    <Shell>
      {header}
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-white px-5 py-3 shadow-sm"><div><span className="font-black">나 {me?.foundCount ?? 0}/3</span><span className="mx-3 text-slate-300">vs</span><span className="font-black">{opponent?.nickname} {opponent?.foundCount ?? 0}/3</span></div>{remaining !== null && <span className={`flex items-center gap-2 rounded-full px-4 py-2 font-black ${remaining <= 10 ? "bg-red-100 text-red-600" : "bg-violet-100 text-violet-700"}`}><Clock size={18}/>{remaining}초</span>}</div>
      ...
      {game.snapshot.state === "EDITING" && <section><div className="mb-4 text-center"><h2 className="text-2xl font-black">차이점 3개를 만드세요</h2><p className="text-sm text-slate-500">그림을 클릭하면 추가·가리기·색상 변경이 순서대로 배치됩니다.</p></div><div className="mx-auto max-w-3xl"><ImageBoard differences={draft} onSelect={addDraft}/></div><div className="mt-5 flex justify-center gap-3"><button disabled={!draft.length || Boolean(me?.submitted)} onClick={() => setDraft((current) => current.slice(0, -1))} className="rounded-xl bg-white px-5 py-3 font-bold shadow disabled:opacity-40">되돌리기</button><button disabled={draft.length !== 3 || Boolean(me?.submitted)} onClick={() => game.submit(draft)} className="inline-flex items-center gap-2 rounded-xl bg-violet-600 px-6 py-3 font-black text-white disabled:opacity-40"><Send size={18}/>{me?.submitted ? "제출 완료 · 상대 대기 중" : `제출 ${draft.length}/3`}</button></div></section>}
      ...
      {game.snapshot.state === "FINISHED" && <section className="mx-auto max-w-2xl rounded-3xl bg-white p-10 text-center shadow-xl"><div className="text-7xl">{game.snapshot.winnerId === game.match.playerId ? "🏆" : game.snapshot.winnerId ? "😿" : "🤝"}</div><h2 className="mt-4 text-3xl font-black">{game.snapshot.winnerId === game.match.playerId ? "승리했습니다!" : game.snapshot.winnerId ? "아쉽게 패배했습니다" : "무승부입니다"}</h2><div className="mt-6 grid grid-cols-2 gap-3"><div className="rounded-2xl bg-violet-50 p-4"><p className="text-sm text-slate-500">내 결과</p><p className="text-xl font-black">{me?.foundCount}/3 · 오답 {me?.wrongAnswerCount}</p></div><div className="rounded-2xl bg-slate-50 p-4"><p className="text-sm text-slate-500">상대 결과</p><p className="text-xl font-black">{opponent?.foundCount}/3 · 오답 {opponent?.wrongAnswerCount}</p></div></div><button onClick={game.returnToLobby} className="mt-7 inline-flex items-center gap-2 rounded-2xl bg-violet-600 px-7 py-4 font-black text-white"><RotateCcw size={19}/>로비로 돌아가기</button></section>}
