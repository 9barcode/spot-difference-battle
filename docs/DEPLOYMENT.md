...
운영 빌드는 Fastify, Socket.IO, React 정적 파일을 하나의 웹 서비스에서 같은 도메인으로 제공합니다. 브라우저는 별도 `VITE_SERVER_URL` 없이 현재 주소의 Socket.IO에 연결합니다. Supabase PostgreSQL은 필수이며 컨테이너 시작 전에 마이그레이션을 적용합니다.
...
4. Render 환경 변수 `SUPABASE_DB_URL`에 Supabase Dashboard의 PostgreSQL 연결 문자열을 등록합니다.
5. 웹 서비스의 첫 배포가 성공할 때까지 기다립니다.
6. 생성된 `https://...onrender.com/health`가 `status: ok`를 반환하는지 확인합니다.
7. PC와 모바일에서 동일 문제 순서·사전 로드·동시 시작·독립 문제 이동·최종 점수 동기화를 확인합니다.
...
- Supabase 무료 플랜의 데이터베이스·연결·Realtime 사용량 제한을 주기적으로 확인합니다.
...
브라우저에서 `http://localhost:3001`과 `http://localhost:3001/health`를 확인합니다. PostgreSQL 복구까지 확인하려면 `SUPABASE_DB_URL`을 컨테이너에 전달합니다. 컨테이너는 서버 시작 전에 마이그레이션을 실행하며, 연결 정보가 없거나 마이그레이션이 실패하면 시작하지 않습니다.
