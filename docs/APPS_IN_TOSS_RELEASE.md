...
최신 로컬 실행 결과와 미검증 항목은 [검증 증거 원장](VERIFICATION_EVIDENCE.md)에서 관리한다. 앱인토스 제출 판단은 [1.0.0 출시 승인 체크리스트](release/APPS_IN_TOSS_1_0_0.md)를 사용한다. `.ait`, 콘솔, 등급분류, 실기기와 운영 환경 증거가 없으면 로컬 자동 검증 결과와 무관하게 승인 준비 완료가 아니다.
