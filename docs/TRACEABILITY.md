...
| AIT-001 | SDK 3 설정, HTTPS·CORS, 게임 내비게이션·가로 화면과 `.ait` 빌드 | `APPS_IN_TOSS_RELEASE` | `apps-in-toss.config.ts`, `vite.config.ts`, URL·CORS 정책 | 타입·단위·E2E·일반 빌드 | 부분: 실제 콘솔 값 `.ait`와 QR 검수 필요 |
