...
import { Clock, Flag, LoaderCircle, LogOut, Minus, Move, Plus, RotateCcw, TimerReset, UsersRound, WifiOff, X } from "lucide-react";
...
const SoloGame = lazy(() => import("../features/solo/components/SoloGame").then((module) => ({ default: module.SoloGame })));
...
export default function App() {
  ...
  if (soloActive) return <Suspense fallback={<Shell><p role="status">솔로 게임을 불러오는 중입니다.</p></Shell>}><SoloGame onExit={() => setSoloActive(false)}/></Suspense>;

  if (game.phase === "LOBBY") return <Shell>{header}<section className="mx-auto max-w-3xl rounded-3xl bg-violet-700 p-6 text-center text-white shadow-xl sm:p-10"><div className="text-6xl">⚔️</div><h2 className="mt-4 text-3xl font-black">동시 빨리찾기</h2><p className="mt-2 text-violet-100">같은 설정을 선택한 상대와 공정하게 대결합니다.</p><button data-testid="solo-mode-open" type="button" onClick={() => setSoloActive(true)} className="mt-6 inline-flex w-full items-center justify-center gap-3 rounded-2xl border-2 border-cyan-300 bg-cyan-950/60 px-6 py-4 text-left"><TimerReset size={24}/><span><strong className="block text-lg">혼자 기록 세우기</strong><small className="text-cyan-100">신규 하드 퍼즐 5세트 · 차이 5개 · 개인 최고기록</small></span></button><div className="mt-6 text-left"><p className="mb-2 font-black">게임 모드</p><div className="grid gap-2 sm:grid-cols-3">{([['STANDARD','기본전','3분 동안 가장 많이 찾기'],['SPRINT','속도전','60초 초고속 대결'],['SURVIVAL','생존전','오답 3번이면 즉시 패배']] as const).map(([value,label,description]) => <button key={value} type="button" onClick={() => setMode(value)} className={`rounded-2xl border-2 p-4 text-left ${mode === value ? 'border-amber-300 bg-white text-violet-800' : 'border-white/20 bg-white/10'}`}><strong className="block">{label}</strong><span className="text-xs opacity-80">{description}</span></button>)}</div></div><div className="mt-5 text-left"><p className="mb-2 font-black">난이도</p><div className="grid gap-2 sm:grid-cols-3">{([['EASY','쉬움','넓은 판정 · 0.5초 잠금'],['NORMAL','보통','기본 판정 · 1초 잠금'],['HARD','어려움','정밀 판정 · 2초 잠금']] as const).map(([value,label,description]) => <button key={value} type="button" onClick={() => setDifficulty(value)} className={`rounded-2xl border-2 p-3 text-left ${difficulty === value ? 'border-amber-300 bg-white text-violet-800' : 'border-white/20 bg-white/10'}`}><strong className="block">{label}</strong><span className="text-xs opacity-80">{description}</span></button>)}</div></div><div className="mt-5 grid gap-3 text-sm sm:grid-cols-3"><span className="rounded-xl bg-white/10 p-3">문제당 차이 3개</span><span className="rounded-xl bg-white/10 p-3">{GAME_MODE_RULES[mode].durationSeconds}초 대결</span><span className="rounded-xl bg-white/10 p-3">오답 잠금 {GAME_DIFFICULTY_RULES[difficulty].wrongAnswerLockSeconds}초</span></div><p className="mt-4 text-sm font-bold text-violet-100">승패: 완료한 그림 수 → 현재 그림에서 찾은 수 → 오답이 적은 순</p><button data-testid="matchmaking-start" disabled={!game.connected} onClick={() => game.startMatching({ mode, difficulty })} className="mt-8 inline-flex items-center gap-2 rounded-2xl bg-amber-400 px-8 py-4 font-black text-slate-900 disabled:opacity-40"><UsersRound size={20}/>{game.connected ? "이 설정으로 상대 찾기" : "서버 연결 중"}</button></section></Shell>;
  ...
  const statusBar = <div className="mx-auto mb-5 flex max-w-6xl flex-wrap items-center justify-between gap-3 rounded-2xl bg-white px-5 py-3 shadow-sm"><div><span className="rounded-full bg-violet-100 px-3 py-1 text-xs font-black text-violet-700">문제 {Math.min((me?.puzzleIndex ?? 0) + 1, game.snapshot.totalPuzzleCount)}/{game.snapshot.totalPuzzleCount}</span><span className="ml-3 font-black">나 {me?.completedAllPuzzles ? "완료" : `${(me?.completedPuzzleCount ?? 0) + 1}/${game.snapshot.totalPuzzleCount}번`} · {me?.foundCount ?? 0}/{me?.currentDifferenceCount ?? 0}</span><span className="ml-3 text-sm text-slate-500">상대 {opponent?.completedAllPuzzles ? "완료" : `${(opponent?.completedPuzzleCount ?? 0) + 1}/${game.snapshot.totalPuzzleCount}번`} · {opponent?.foundCount ?? 0}/{opponent?.currentDifferenceCount ?? 0}</span></div><div className="flex items-center gap-2">{remaining !== null && <span className={`flex items-center gap-2 rounded-full px-4 py-2 font-black ${remaining <= 10 ? "animate-pulse bg-red-100 text-red-600" : remaining <= 30 ? "bg-amber-100 text-amber-700" : "bg-violet-100 text-violet-700"}`}><Clock size={18}/>{remaining}초</span>}{!['FINISHED','CANCELLED'].includes(game.snapshot.state) && <button data-testid="forfeit-button" onClick={() => window.confirm("경기를 나가면 기권패입니다. 나갈까요?") && game.forfeit()} className="rounded-full bg-slate-100 p-2 text-slate-500"><LogOut size={18}/></button>}</div></div>;

  return <Shell>{header}{statusBar}
    {game.snapshot.state === "PLAYING" && (me?.correctStreak ?? 0) >= 2 && <div className="mx-auto mb-4 max-w-sm animate-pulse rounded-2xl bg-orange-100 px-5 py-3 text-center text-xl font-black text-orange-700">🔥 {me?.correctStreak}연속 정답!</div>}
