...
import { ACTIVE_GAME_PUZZLES } from "./puzzle-catalog.js";
...
export class MatchRegistry {
  ...
  constructor(
    private readonly puzzles: readonly MatchPuzzle[] = ACTIVE_GAME_PUZZLES,
  ) {
    if (puzzles.length === 0) {
      throw new Error("하나 이상의 게임 문제가 필요합니다.");
    }
  }
  ...
  create(
    ...
    // MatchRegistry에 주입된 퍼즐 목록을 복사한 뒤
    // 각 경기마다 랜덤한 순서로 사용한다.
    // Supabase puzzle_catalog에서 가져온 퍼즐도 동일하게 처리된다.
    const selected = this.puzzles
      .map((puzzle) => ({
